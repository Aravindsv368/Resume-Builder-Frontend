...
const EditResume = () => {
    ...
    switch (currentPage) {
      ...
      case "skills":
        resumeData.skills.forEach(({ name, progress }, index) => {
          if (!name.trim())
            errors.push(`Name is required in skills ${index + 1}`);
      ...
      case "projects":
        resumeData.projects.forEach(({ title, description }, index) => {
          ...
          if (!description.trim())
            errors.push(
              `Project description is required in skill ${index + 1}`
            );
        });
        break;
      case "certifications":
        resumeData.certifications.forEach(({ title, issuer }, index) => {
          if (!title.trim())
            errors.push(`Title is required in project ${index + 1}`);
          if (!issuer.trim())
            errors.push(`Issuer is required in skill ${index + 1}`);
        });
        break;
      case "additionalInfo":
        if (
          !resumeData.languages.length === 0 ||
          !resumeData.languages[0]?.trim()
        ) {
        ...
        if (
          ...
          errors.push(`At least ond interest is required  ${index + 1}`);
        }
        break;
      default:
        break;
  ...
  const uploadResumeImages = async () => {
    try {
      ...
      // Call the second API to Upload other resume data
  ...
  const handleDeleteResume = async () => {
    try {
      setIsLoading(true);
      const response = axiosInstance.delete(API_PATHS.RESUME.DELETE(resumeId));
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto">
        <div className="flex items-center justify-between gap-5 mx-20 bg-white rounded-lg border border-purple-100 py-3 px-4 mb-4">
          <TitleInput
